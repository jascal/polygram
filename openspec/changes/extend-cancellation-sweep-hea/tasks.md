...
- [ ] 1.1 `polygram/dictionary.py` — `Dictionary.with_knob(path, value)`
...
- [ ] 1.2 `<feature>.phi` on `MPSRung1` updates `Feature.phi`
      ...
      untouched.
- [ ] 1.3 `<feature>.theta[r,d,q]` is rejected on `MPSRung1` with
      `ValueError` naming the encoding.
- [ ] 1.4 `<feature>.theta[r,d,q]` on `HEA_Rung2`: when
...
- [ ] 3.1 `Cancellation.structural_floor()` raises
      ...
      paragraph.
- [ ] 3.2 `cancellation_efficiency` is `None` when
...
- [ ] 3.3 `CancellationResult` materialized summary distinguishes
...
- [ ] 5.1 `polygram/_assertions.py` — new
...
- [ ] 5.2 `Experiment.__post_init__` rejects the assertion at
      construction time when the dictionary's encoding has no
      `tier_separation_bound` (either MPS rung-1, or HEA with the
      bound set to `None`).
- [ ] 5.3 `SUPPORTED_ASSERTIONS` extended; `_assertions.py`
      `__all__` updated.

## 6. Combined before/after plot

- [ ] 6.1 `polygram/cancellation.py` — `CancellationResult.plot`
...
- [ ] 6.2 Tier-separation indicator on the bar chart: if the
...
- [ ] 6.3 Both Gram heatmaps highlight the target pair cell with
      a marker so it's recognizable at a glance.

## 7. Tests

### Dictionary
- [ ] 7.1 `tests/test_dictionary.py::TestWithKnob` — `.phi` on MPS
      and HEA, `.theta[r,d,q]` on HEA happy path, `.theta` rejected
      on MPS, out-of-range slot rejected, malformed path rejected.

### Cancellation
- [ ] 7.2 `tests/test_cancellation.py::TestKnobsList` — default
...
- [ ] 7.3 `tests/test_cancellation.py::TestStructuralFloorContract`
...
### InterferenceSweep
- [ ] 7.4 `tests/test_experiment.py::TestSweepKnobs` — `.phi` axis
      on HEA dictionary works; `.theta[r,d,q]` axis on HEA works;
      `.theta` axis on MPS rejected.
- [ ] 7.5 `tests/test_experiment.py::TestTierSeparationMeasure` —
...
- [ ] 8.1 `examples/animals_hea.py` — extend `main()` to (a) run
      ...
      first.
- [ ] 8.2 Example output directory layout documented in the
      module docstring.

## 9. Validate + commit

- [ ] 9.1 `openspec validate extend-cancellation-sweep-hea --strict`
      ✓
- [ ] 9.2 Full pytest suite green; ruff clean
