...
Target-feature-count compression and Pareto-path planning. Lets a
caller request *"compress this SAE to ~K cluster representatives"*
directly (rather than tuning thresholds until K falls out), and
lets them request a nested sequence of plans across many K values
from a single sort of the candidate pair list. The historical
threshold-driven `Compressor.plan()` path is unchanged and remains
the default.

*Semantic note*: "K" throughout this spec refers to
`CompressionReport.n_features_kept`, which is the count of cluster
*representatives* (one survivor per cluster), not the total
surviving feature count of the SAE checkpoint. See Decision 1 in
`design.md`.
