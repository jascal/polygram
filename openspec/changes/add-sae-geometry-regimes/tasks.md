...
- [ ] 1.1 Create `polygram/geometry/` package with `__init__.py`, `profile.py`, `protocols.py`, `registry.py`, `text_clustered.py`, `uniform_sphere.py`
...
## 2. Extract the v0.1.0 path into the text-clustered profile

- [ ] 2.1 Move today's `_kmeans`, `_spread_betas`, `_centroids`, `_variance_explained`, `_gamma_via_cluster_pca` helpers from `polygram/sae_import.py` into `polygram/geometry/text_clustered.py` (or import them from there) — preserve their signatures and float behaviour
- [ ] 2.2 Implement `TextClusteredKnobAssignment.assign(...)` that wraps those helpers and returns a `KnobAssignmentResult` with `cluster_method="kmeans"`
- [ ] 2.3 Move `_tier_preservation` (today in `sae_import.py`) into `text_clustered.py` as `TierPreservationFidelity.compute(...)` returning `float | None`
- [ ] 2.4 Implement `text_clustered() -> GeometricProfile` factory with `default_n_clusters=2`, `default_gamma_range=(-0.25, 0.25)`
- [ ] 2.5 Generate a frozen golden fixture `tests/fixtures/golden_text_clustered.json` from the v0.1.0 baseline on the toy SAE: capture full `Dictionary.features` (name, cluster, beta, gamma, phi), `report.cluster_assignments`, `report.beta_variance_explained`, `report.tier_preservation`
- [ ] 2.6 Add regression test `tests/test_text_clustered_golden.py` asserting byte-equality of v0.2 output against the golden fixture for a deterministic fixture call
...
- [ ] 4.3 Implement profile resolution at `from_sae_lens` entry: kwarg > config > registry default (`text-clustered`); resolve string names via `get_profile`
...
- [ ] 4.6 Add `profile: str` and `geometric_fidelity: float | None` fields to `SelectionReport`; populate `tier_preservation` only when the active profile's fidelity is the v0.1.0 Pearson (i.e. `text-clustered`)
...
- [ ] 5.2 Add `tests/test_from_sae_lens_profile_dispatch.py`: omitting `profile=` is bit-equal to passing `profile="text-clustered"`; per-field `n_clusters` overrides profile default; `cluster_assignments` bypasses strategy but profile fidelity is still computed
...
- [ ] 6.1 Re-export `GeometricProfile`, `register_profile`, `get_profile`, `available_profiles`, `text_clustered`, `uniform_sphere` from `polygram.__init__`
...
- [ ] 7.2 Update `README.md` "Choosing an encoding" section: cross-link profiles to encodings (text-clustered + MPSRung1 is the validated combo; uniform-sphere is provisional pending behavioural validation)
...
- [ ] 8.3 Confirm `polygram analyze` CLI on the bundled toy SAE produces identical output before and after this change (text-clustered default path)
