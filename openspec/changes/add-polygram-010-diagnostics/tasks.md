...
- [ ] 1.2 Update `polygram.compression.epoch.EpochCompressor._build_report` (or equivalent — find the construction site via `git grep "EpochReport("`) to plumb the SAE's input feature count through and compute `redundancy_ratio = n_features_zeroed_total / max(1, n_features_input)`.
...
- [ ] 4.1 Update `polygram/compression/epoch_report.py` class docstring to describe `redundancy_ratio`'s correlation with downstream KL outcomes (cite the 60–74% range observed across recent runs).
