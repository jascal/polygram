...
- [ ] 1.1 New module `polygram/batch.py` with `BatchRun` frozen
...
- [ ] 1.2 `BatchResults` frozen dataclass with the field set named
      in the proposal (`source_graph`, `dictionary_name`, `knobs`,
      `created_at`, `runs`).
- [ ] 1.3 `BatchResults.to_json(path)` — deterministic ordering
...
- [ ] 2.2 `_resolve_knob_paths(self, edge) -> list[str]` — for
...
- [ ] 2.3 `BatchExperiment.run() -> BatchResults` — for the first
...
- [ ] 3.2 `--feature-graph` parsed via `FeatureGraph.from_json`
...
- [ ] 3.3 `--dictionary` accepted as either a `.q.orca.md` file
      ...
      `Dictionary`.
- [ ] 3.4 Construct `BatchExperiment`; surface `__post_init__`
...
- [ ] 5.2 Module docstring documents the output layout and the
      prediction-vs-observation comparison the user can run.

## 6. Tests

- [ ] 6.1 `tests/test_batch.py::TestBatchRun` — frozen,
      `cancellation_efficiency` is `0.0` when `predicted_gap == 0`.
- [ ] 6.2 `tests/test_batch.py::TestBatchResults` — JSON
...
- [ ] 6.3 `tests/test_batch.py::TestBatchExperiment` — pair set is
...
- [ ] 6.4 `tests/test_cli.py::TestBatchSubcommand` — end-to-end on
...
- [ ] 7.1 `tech-debt-backlog` — new bullet capturing the
