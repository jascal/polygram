...
- [ ] 2.3 `FeatureGraph.to_json()` — `json.dumps` with
...
- [ ] 3.2 `build_separation_graph(prediction, *, threshold=0.2,
      include_within_cluster=False) -> FeatureGraph` — same shape
      with the separation edge-weight rule.
      `metadata["kind"] = "separation"`.
- [ ] 3.3 Singleton features (no kept edge) appear as size-1
      components for both kinds.

## 4. Renderer

- [ ] 4.1 `render_feature_graph_section(graph) -> str` — kind-aware
...
- [ ] 4.2 Section is deterministic given the input.
...
- [ ] 6.1 `polygram/cli.py` — `_cmd_analyze` parser gains four
      flags (sharing-graph/threshold + separation-graph/threshold).
- [ ] 6.2 Handler: when each `*-graph` flag is supplied, call the
...
- [ ] 7.5 `tests/test_cli.py::test_analyze_emits_sharing_graph` and
      ...
      right `"kind"`.
- [ ] 7.6 `tests/test_cli.py::test_analyze_threshold_malformed` —
