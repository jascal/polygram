...
The subcommand SHALL accept two additional optional flags:

- `--sharing-graph <path.json>` — when supplied, the handler SHALL
  invoke `polygram.analysis.build_sharing_graph` on the prediction
  and write the result of `SharingGraph.to_json()` to the named
  path. When omitted, no sharing graph is produced and existing
...
`--features` argument, malformed `--sharing-threshold` value, or any
error raised by `predict_cancellation_depth` or
`build_sharing_graph`.
...
- **AND** the parsed dict has the keys `"nodes"`, `"edges"`,
