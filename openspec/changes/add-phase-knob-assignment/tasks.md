...
- [ ] 13.1b Bump `polygram/__init__.py::__version__` to `0.5.0` (minor) — new opt-in capability, backward-compat default. Reviewer flagged this as part of the merge gating; the PR-#63-era amp-axis shift is the load-bearing reason the version must bump (callers depending on exact gram-condition numbers see different output even with the same flag value).
