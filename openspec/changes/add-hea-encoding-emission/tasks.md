...
- [x] 1.1 `pyproject.toml` — bump `q-orca>=0.7.1` to `q-orca>=0.9.0`
- [x] 1.2 Confirm `pip install -e ".[dev]"` resolves against the
      published v0.9.0 wheel
- [x] 1.3 `AGENTS.md` — update the Q-Orca dependency-contract
      paragraph to mention HEA + tier invariants are now part of
      the stable surface

## 2. Encoding marker

- [x] 2.1 `polygram/encoding.py` — `HEA_Rung2` frozen dataclass with
      fields `depth: int`, `entangler: str = "ring"`,
      `rotations: tuple[str, ...] = ("Ry", "Rz")`,
      `tier_separation_bound: float | None = 0.025`,
      `n_qubits: int = 3`
- [x] 2.2 `__post_init__` validates `depth >= 1`,
      `entangler in {"ring", "chain"}`, every rotation in
      `{"Rx", "Ry", "Rz"}`, and (if not None)
      `0.0 <= tier_separation_bound <= 1.0`
- [x] 2.3 Public re-export from `polygram.__init__` alongside
      `MPSRung1`

## 3. Feature θ tensor

- [x] 3.1 `polygram/dictionary.py` — `Feature` gains optional
      `theta: np.ndarray | None = None`. Default `None` means
      "let the emitter generate a tensor from `(α, β, γ, φ)`."
- [x] 3.2 New helper `_default_hea_theta(feature, encoding)` builds
...
- [x] 5.1 `_qorca_emit.render_machine_markdown` dispatches on
      `dictionary.encoding`. Existing rung-1 path stays bit-for-bit
      identical.
- [x] 5.2 HEA branch emits a `## encoding` table with `kind: hea`,
      `depth`, `entangler`, `rotations` — matches the q-orca-lang
      `examples/larql-hea-minimal.q.orca.md` shape.
- [x] 5.3 HEA branch emits a 3-column `## theta` table
      ...
      field verbatim.
- [x] 5.4 HEA branch emits `## invariants` with
...
- [x] 5.5 The HEA branch emits a single `query_concept` action and
...
- [x] 6.1 `tests/test_encoding.py::TestHEARung2` — depth/entangler/
      rotations/bound validation, defaults, frozen-dataclass
      equality.

### Dictionary

- [x] 6.2 `tests/test_dictionary.py::TestHEADictionary` — accept
...
- [x] 6.3 `tests/test_qorca_emit.py::TestHEAEmit` — emitted markdown
...
- [x] 6.4 `tests/test_qorca_emit.py::TestHEAEmit::test_no_invariant`
      — passing `tier_separation_bound=None` produces no
      `## invariants` section.
- [x] 6.5 `tests/test_emit.py` — `write_qorca` round-trips an
...
- [x] 7.1 `examples/animals_hea.py` — same Animals dictionary as
...
- [x] 7.2 The original `examples/animals.py` is untouched —
      additive change.

## 8. Validate + commit

- [x] 8.1 `openspec validate add-hea-encoding-emission --strict` ✓
